import { type BriefNews, type NewsCategory } from "../types/brief_news.entity.js";
...
export class NewsReader {
    ...
    public async read(all_categories: Map<NewsCategory, Map<string, BriefNews>>): Promise<Map<NewsCategory, Map<string, BriefNews>>> {
        ...
        for (let i = 0; i < categories.length; i++) {
            ...
            if (result.status === 'rejected')
                throw result.reason;
            const items: Map<string, BriefNews> = result.value;
